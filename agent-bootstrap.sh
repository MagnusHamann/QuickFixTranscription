...
VENV_DIR="$DEPENDENCY_ROOT/.venvs/QuickFixTranscription"
cd "$APP_DIR"
...
install_linux_prereqs() {
    ...
    if command -v brew >/dev/null 2>&1; then
        brew install whisper-cpp || echo "whisper-cpp Homebrew install did not complete. You can still choose a local executable in the app."
    else
        echo "whisper-cpp was not auto-installed on Linux because Homebrew is unavailable."
        echo "Install whisper-cpp manually or choose a local executable in the app."
    fi
}
