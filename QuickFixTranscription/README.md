...
The default model is downloaded during setup/launch from the `ggml-org/whisper.cpp` model distribution on Hugging Face and verified by SHA1 checksum before use. It is a local model file after download; recordings are still processed locally.

You must also provide a local `whisper.cpp` executable. The app checks common local locations such as:
